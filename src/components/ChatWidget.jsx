...
import { getChatSessionId, sendChatMessage } from '../lib/chat';

const GREETING =
  "Hi! I'm the Leora Wellness assistant. Ask me about our products, ingredients, shipping, or returns.";

// Global floating widget (functional-requirements.md §4.2), mounted once in
// App.jsx so it's present on every route across all three storefronts —
// unlike ShopCart, this isn't scoped to a brand/page. Bottom-left on purpose:
// ShopCart's own floating button/bar already owns the bottom-right corner
// (desktop) and the full-width bottom bar (mobile), so this sits at
// bottom-24 on mobile to clear that bar regardless of whether it's showing,
// and bottom-6 opposite the cart FAB on desktop.
const ChatWidget = () => {
  ...
  // A stable per-browser id, not a Supabase auth session — lets the
  // Edge Function keep one continuous conversation per visitor. See
  // src/lib/chat.js for why this isn't just supabase.auth's session.
  useEffect(() => {
    ...
    <>
      {!isOpen && (
        <motion.button
          ...
          className="fixed z-40 bottom-24 left-4 lg:bottom-6 lg:left-6 h-14 w-14 rounded-full bg-primary text-white inline-flex items-center justify-center shadow-[0_14px_32px_rgba(31,44,35,0.32)] hover:bg-forest-800 transition-colors"
        >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed z-[95] bottom-24 left-4 right-4 lg:right-auto lg:left-6 lg:bottom-6 lg:w-[380px] h-[65vh] max-h-[520px] lg:h-[560px] lg:max-h-[75vh] rounded-2xl bg-white shadow-[0_24px_60px_rgba(8,14,10,0.28)] border border-[var(--color-border-light)] flex flex-col overflow-hidden"
          >
